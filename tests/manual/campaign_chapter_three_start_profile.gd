extends SceneTree
## Produce an explicitly labelled bot-completed chapter-one-plus-two sample, without
## synthetic unlocks. The chapter-three freeze package seeds its first launch from it.
...
func _run():
	...
	for index in 16:
		...
		a.free()
	assert(p.data.completed==16 and p.data.current_run==null)
	print("CHAPTER_THREE_START_PROFILE_PASS completed=16 branches=",p.data.branches," pages=",p.data.pages)
	s.free()
	quit()
