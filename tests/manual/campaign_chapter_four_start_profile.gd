extends SceneTree
## Produce an explicitly labelled bot-completed first-three-chapters sample, without
## synthetic unlocks. The full-campaign freeze package seeds its first launch from it.
...
func _run():
	...
	for index in 24:
		for branch in 3:
			if p.data.pages>=4 and p.data.branches[branch]<(1 if index<8 else 2): assert(p.purchase_branch(branch))
		...
		a.free()
	assert(p.data.completed==24 and p.data.current_run==null)
	print("CHAPTER_FOUR_START_PROFILE_PASS completed=24 branches=",p.data.branches," pages=",p.data.pages)
	s.free()
	quit()
