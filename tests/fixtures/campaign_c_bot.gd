extends RefCounted
## Same avoidance scoring, with ordered clue navigation for the new hunt objective.
	...
	for id in a.state.offered:
		...
		if "--qa-production-evolution" in OS.get_cmdline_user_args():
			if id in ["S1-A01", "S1-P01"]:
				candidate = 95.0
		if "--alternate-build" in OS.get_cmdline_user_args():
			if id == "S1-A02": candidate = 120.0
			elif id == "S1-A03": candidate = 115.0
		if candidate > score:
			score = candidate
			best = id
	return best
...
static func direction(a, m: Dictionary, tick_value: int) -> Vector2:
	...
	var kind := str(m.kind)
	if m.has("clues") and a.state.encounter.chapter.clues < 3:
		var clue: Array = m.clues[int(a.state.encounter.chapter.clues)]
		target = Vector2(clue[0],clue[1])
	elif kind == "SURVIVE":
		...
		# Follow the visible late-boss teaching: step outside the spore/anchor ring.
		# A fixed 250px standoff puts this bot inside those advertised danger bands.
		var standoff := 400.0 if m.has("late_boss") and m.late_boss.pattern in ["spore_tide","anchors"] else 250.0
		if away.length() < standoff-20.0:
			target += (away.normalized() if away.length() > 0 else Vector2.LEFT) * standoff
	...
	for candidate_index in 17:
		...
		var horizon: float = minf(speed*0.5,maxf(10,player.distance_to(target))) if m.has("clues") and a.state.encounter.chapter.clues < 3 else speed*0.5
		var predicted: Vector2 = player + direction * horizon
