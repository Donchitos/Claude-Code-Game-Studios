...
var evidence: String = preload("res://tests/fixtures/campaign_evidence.gd").create("journey-b")
var failed := false
var rows: Array = []
...
func _run() -> void:
	...
	var stem := OS.get_temp_dir()+"/package_b_journey_%d" % Time.get_ticks_usec()
	...
	for index in 3:
		...
		for tick in 30000:
			...
			var stages := JSON.stringify(arena.state.encounter.stage_ticks)
		...
		rows.append(row)
		print("B_JOURNEY ",JSON.stringify(row))
		...
		arena.free()
	var file := FileAccess.open(evidence+"journey.json",FileAccess.WRITE)
	file.store_string(JSON.stringify({"catalog_hash":c.content_hash,"missions":rows,"completed":profile.data.completed,"scope":"real new-profile sequential three missions, bot not human"},"\t"))
	file.close()
	failed = failed or profile.data.completed != 3
	storage.free()
	DirAccess.remove_absolute(stem+"a")
	DirAccess.remove_absolute(stem+"b")
	print("PACKAGE_B_JOURNEY_", "FAIL" if failed else "PASS")
