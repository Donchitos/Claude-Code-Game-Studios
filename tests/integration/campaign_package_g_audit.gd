...
var evidence: String = preload("res://tests/fixtures/campaign_evidence.gd").create("g-pacing-audit")
...
func _run() -> void:
	...
	for index in 8:
		...
		var hunt_phases: Array = []
		var previous_hunt := -1
		var warnings: Array = []
		var observed_landings: Array = []
		var landing_phases := {}
		for tick in 30000:
			...
			var old_landings: Array = a.state.encounter.get("chapter",{}).get("landings",[]).duplicate(true)
			a.advance(1.0/60,Bot.direction(a,c.missions[index],tick))
			var chapter: Dictionary = a.state.encounter.get("chapter",{})
			for landing in chapter.get("landings",[]):
				var id := JSON.stringify(landing,"",true,true)
				if not landing_phases.has(id):
					landing_phases[id] = int(chapter.boss_phase)
					warnings.append({"tick":a.state.tick,"phase":chapter.boss_phase,"due_tick":landing.tick})
			for landing in old_landings:
				if landing.tick <= a.state.tick and a.state.entities.any(func(e): return e.family == "boss" and e.hp > 0):
					observed_landings.append({"tick":a.state.tick,"warning_phase":landing_phases.get(JSON.stringify(landing,"",true,true),-1)})
			if a.mission.has("clues"):
				for entity in a.state.entities:
					if entity.target_id != "" and int(entity.phase) != previous_hunt:
						hunt_phases.append({"tick":a.state.tick,"phase":entity.phase})
						previous_hunt = int(entity.phase)
			var near := false
			var live := 0
		...
		rows.append({"mission":a.mission.id,"kind":a.mission.kind,"elapsed":a.state.elapsed,"choices":choices,"hunt_phases":hunt_phases,"boss_warnings":warnings,"boss_observed_landings":observed_landings,"objective_changes":changes,"planned":planned,"attempted":attempted,"skipped":a.state.encounter.skipped,"stages":stages,"nearby_enemy_seconds":float(nearby_ticks)/60,"hostile_zone_seconds":float(hostile_zone_ticks)/60,"max_live_non_target":max_enemies,"kills":a.state.player.kills,"damage_taken":a.state.statistics.damage_taken,"chapter":a.state.encounter.get("chapter",{}),"evolved":a.state.evolved})
		assert(p.finish_run(true,a.stats()).status == "OK")
		a.free()
	assert(p.data.completed == 8)
	var file := FileAccess.open(evidence+"g-pacing-audit.json",FileAccess.WRITE)
	file.store_string(JSON.stringify({"catalog_hash":c.content_hash,"alternate":alternate,"rows":rows,"scope":"landing observer counts due entries while boss survives; excludes lethal-tick ambiguity. natural bot telemetry; nearby means live non-target within 320 world units; hostile zones may be remote or warning; neither is human engagement"},"\t"))
	file.close()
	s.free()
