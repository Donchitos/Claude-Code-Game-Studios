...
# This suite isolates transactional storage using a deliberately minimal catalog.
# Real Arena semantics and run identity are exercised in campaign_package_d_fix_test.
class TransactionProfile extends Profile:
	func save_run(snapshot: Dictionary) -> bool:
		var fixture := snapshot.duplicate(true)
		fixture["state"] = {}
		fixture["numeric_bits"] = {}
		return super.save_run(fixture)

	func _valid_checkpoint(_snapshot: Dictionary) -> bool:
		return true
...
func _catalog() -> Dictionary:
	var c := {"content_hash": "a".repeat(64), "missions": [], "characters": [{"id": "hero", "unlock_after": 0}, {"id": "sage", "unlock_after": 1}], "skills": [], "passives": [], "evolutions": [], "events": [], "pills": [{"id": "heal", "unlock_after": 0, "cost": 2, "stat": "max_hp", "amount": 10}], "achievements": [], "challenges": [], "tuning": {"progression_unlock_completed": [0, 8, 24, 40, 56], "progression_cost_base": 4, "progression_cost_step": 4, "profile_pill_cap": 99, "profile_replay_reward": 2, "profile_kills_per_page": 25, "profile_battle_reward_cap": 4, "profile_first_herbs": 3, "profile_kills_per_herb": 20, "profile_battle_herb_cap": 3}}
	for i in 64:
...
func _run() -> void:
	...
	var copy := TransactionProfile.new()
...
func _test_disk(c: Dictionary) -> void:
	...
	p = TransactionProfile.new()
	_expect(p.initialize(c, s) and preload("res://src/campaign/campaign_arena_validation.gd").same_values(p.data,before), "reload exact afterimage")
	...
	p = TransactionProfile.new()
	_expect(p.initialize(c, s) and preload("res://src/campaign/campaign_arena_validation.gd").same_values(p.data,before), "previous durable checkpoint survives corrupt latest")
	var changed := c.duplicate(true)
	changed.content_hash = "b".repeat(64)
	var mismatch := TransactionProfile.new()
...
func _test_boundaries(c: Dictionary) -> void:
	...
	before = p.data
	var stale := TransactionProfile.new()
	...
	var capped := TransactionProfile.new()
...
func _test_faults(c: Dictionary) -> void:
	...
	var unavailable := TransactionProfile.new()
...
func _test_challenges(c: Dictionary) -> void:
	...
	var reload := TransactionProfile.new()
...
func _test_event_rewards() -> void:
	...
	for choice: String in ["safe", "risk"]:
		...
		var resumed := TransactionProfile.new()
		...
		s.fail = false
		resumed = TransactionProfile.new()
...
func _test_fullscreen() -> void:
	...
	var restored := TransactionProfile.new()
	...
	s.fail = false
	restored = TransactionProfile.new()
...
func _test_fullscreen_migration() -> void:
	...
	for inject_failure: bool in [false, true]:
		...
		var migrated := TransactionProfile.new()
		...
		if inject_failure:
			...
			s.fail = false
			migrated = TransactionProfile.new()
			success = migrated.initialize(c, s)
		var expected := old.duplicate(true)
		expected.settings["fullscreen"] = false
		_expect(success and preload("res://src/campaign/campaign_arena_validation.gd").same_values(migrated.data,expected), "migration preserves all progress seed loadout and offered choices")
		...
		migrated = TransactionProfile.new()
		_expect(migrated.initialize(c, s) and preload("res://src/campaign/campaign_arena_validation.gd").same_values(migrated.data,expected), "migrated seven-setting profile reloads from disk")
		var generation: Variant = s.profile_snapshot().generation
		var again := TransactionProfile.new()
		...
		for variant: int in [0, 1, 2]:
			...
			var bad := TransactionProfile.new()
