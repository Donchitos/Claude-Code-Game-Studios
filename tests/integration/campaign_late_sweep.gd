...
func _run():
	...
	for index in range(24,64):
		for run_seed in [711,977,1345899233]:
			...
			a.free()
	FileAccess.open("res://production/playtest-evidence/final-chapters-2026-09-22/sweep.json",FileAccess.WRITE).store_string(JSON.stringify({"hash":c.content_hash,"rows":rows,"wins":wins,"scope":"zero growth C01 bot fixtures, not human"},"\t"))
