extends SceneTree
## Twenty-four real missions, legal fresh progression, disk resume at stage/choice boundaries.
...
var evidence: String = preload("res://tests/fixtures/campaign_evidence.gd").create("journey-chapter-three")
var failed := false
var rows: Array = []
...
func _run() -> void:
	...
	var stem := OS.get_temp_dir()+"/chapter_three_journey_%d" % Time.get_ticks_usec()
	...
	for index in 24:
		...
		if index >= 8: assert(not profile.purchase_branch(0))
		...
		for tick in 30000:
			...
			if index == 20: tide = int(arena.state.tick) % 450 >= 90
				...
				for entity in arena.state.entities:
					if entity.family == "boss":
						...
						break
			var stages := JSON.stringify([arena.state.encounter.stage_ticks,arena.state.event_id,arena.state.event_done,arena.state.encounter.last_upgrade_tick,arena.state.objective.completed_ids,tide])
			if stages != last_stages and not arena.state.finished:
				...
				if not next.configure(c,c.missions[index],run.loadout,str(run.seed).to_int(),run.snapshot):
					FileAccess.open("/tmp/ch3-failed-restore.json",FileAccess.WRITE).store_string(JSON.stringify({"run":run,"original":snap},"\t",true,true))
		...
		rows.append(row)
		print("C3_JOURNEY ",JSON.stringify(row))
		...
		arena.free()
	# Tier 3 unlocks exactly at 24 completed missions; tier 4 stays gated at 40.
	if profile.data.completed == 24 and profile.data.pages >= 4:
		assert(profile.purchase_branch(0))
	...
	file.store_string(JSON.stringify({"catalog_hash":c.content_hash,"missions":rows,"completed":profile.data.completed,"branches":profile.data.branches,"scope":"real new-profile sequential twenty-four missions, bot not human"},"\t"))
	file.close()
	failed = failed or profile.data.completed != 24
	storage.free()
	DirAccess.remove_absolute(stem+"a")
	DirAccess.remove_absolute(stem+"b")
	print("CHAPTER_THREE_JOURNEY_", "FAIL" if failed else "PASS")
