...
When a task requires deep expertise in a specific Godot subsystem, spawn a
generic sub-agent and load the relevant role profile with the task context:

- `roles/godot-gdscript-specialist.md` — GDScript architecture, static typing, signals, coroutines
- `roles/godot-shader-specialist.md` — Godot shading language, visual shaders, particles
- `roles/godot-gdextension-specialist.md` — C++/Rust bindings, native performance, custom nodes

Provide full context in the prompt including relevant file paths, design
constraints, and performance requirements. Use parallel execution only for
independent tasks.
...
introduced after May 2025, verify it against the current official Godot documentation online.
...
under the `gap` type (GAP programming language). Using `--type gdscript` produces
a hard error — the search never executes.

**Always use a file glob** when filtering GDScript files:
- `rg --glob "*.gd"` ✓  |  `rg --type gdscript` ✗
