...
**RESOLVED by ADR-0018 (2026-07-27)**: `game.showModal('context_menu')` had no production call site anywhere in `src/lib/` — flagged by both code reviewers as a real product gap, not just a documentation inconsistency (Pet Interaction epic's already-ratified GDD/ADR-0016 binds a tap on Mochi's sprite directly to PLEASED, conflicting with this story's own GDD prose). User decision: keep tap-on-Mochi = PLEASED unchanged; open the context menu via a new dedicated `MochiOptionsButton` (a small, persistent, always-visible icon button, not a new Flame gesture) instead of reusing tap or repurposing long-press (which `pet-interaction.md`'s own Open Question #1 reserves for a distinct future interaction). See `docs/architecture/adr-0018-mochi-options-button-context-menu-trigger.md` for the full decision, `src/lib/ui/mochi_options_button.dart` for the implementation, and `tests/integration/pet-room-screen-ui/mochi_options_button_test.dart` for verification. The context menu this story built is now reachable by a real player action.
