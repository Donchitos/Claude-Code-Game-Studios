...
- [x] **AC-08**: Server returns 500 twice then 200; `MAX_FLUSH_RETRIES=3` → delivered on 3rd attempt; 3 HTTP calls; events cleared from queue
- [x] **AC-09**: Same batch sent twice (same `eventId` values) → DB has exactly 1 record per eventId; both requests return 200
- [x] **AC-10**: Player revokes consent; 30 Tier 1 events queued → all Tier 1 events removed from queue and AsyncStorage; Tier 0 events preserved
...
- Story 002: flush trigger mechanics (interval, size, background) already implemented
- Story 004: clock skew and malformed event detection
