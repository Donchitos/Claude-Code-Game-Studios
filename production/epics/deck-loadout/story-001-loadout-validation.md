# Story 001: Loadout Validation (ownership, ability existence, slot count)

> **Epic**: deck-loadout
> **Status**: Complete
> **Layer**: Core
> **Type**: Logic
> **Estimate**: S
> **Manifest Version**: not yet created
> **Last Updated**: 2026-05-29

## Context

**GDD**: `design/gdd/deck-loadout.md`
**Requirement**: `TR-core-???` *(pending `/architecture-review`)*

**ADR Governing Implementation**: ADR-0013
**Engine**: React Native (Expo SDK) + Node.js | **Risk**: LOW
**Engine Notes**: N/A — pure Node.js server logic. No game engine API involved.

---

## Acceptance Criteria

- [x] See GDD `design/gdd/deck-loadout.md` §8 Acceptance Criteria for this story

---

## Test Evidence

**Story Type**: Logic
**Required evidence**: `tests/unit/deck-loadout/deck_loadout_test.ts`

**Status**: [ ] Not yet created

---

## Dependencies

- Depends on: Foundation layer complete
- Unlocks: Story 002