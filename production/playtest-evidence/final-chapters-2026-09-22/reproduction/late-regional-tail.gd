...
func _run():
	...
	var rows=[]
	for index in range(47,64):
		for run_seed in [int(seeds[index])]:
			var a=Arena.new();root.add_child(a)
			var l={"character_id":"S1-C%02d" % (index/8+1),"completed":index,"branches":[4,4,4] if index<56 else [5,5,5],"difficulty":0,"pill_id":"","challenge_id":""}
			...
			a.free()
	FileAccess.open("res://production/playtest-evidence/final-chapters-2026-09-22/regional-tail.json",FileAccess.WRITE).store_string(JSON.stringify({"hash":c.content_hash,"rows":rows,"wins":wins,"scope":"zero growth C01 bot fixtures, not human"},"\t"))
	print("LATE_SWEEP_RESULT ",wins,"/17")
	quit(0 if wins==17 else 1)
