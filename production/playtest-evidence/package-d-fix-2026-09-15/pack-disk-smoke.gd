...
func _run():
	...
	assert(OS.get_user_data_dir().contains("Spirit Nexus D Fix 20260915"))
	...
	root.get_texture().get_image().save_png("/Users/tal/CursorProjects/xiuxian/Claude-Code-Game-Studios/production/playtest-evidence/package-d-fix-2026-09-15/pack-disk-smoke.png")
	game.queue_free()
	await process_frame
	await process_frame
	assert(DirAccess.remove_absolute(slot_a) == OK)
	assert(DirAccess.remove_absolute(slot_b) == OK)
	print("D_FIX_PACK_DISK_SMOKE_PASS 120 ticks saved and reloaded; isolated test slots removed ",OS.get_user_data_dir())
	quit()
