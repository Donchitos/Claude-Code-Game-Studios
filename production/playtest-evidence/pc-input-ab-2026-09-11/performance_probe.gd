...
func _run() -> void:
	...
	for mode: String in modes:
		...
		for tick in WARMUP + SAMPLES:
			...
			var ok: bool = battle.run_tick(1.0 / 60.0, tick + 1)
		...
		row["raw_simulation_ms"] = durations
		row["raw_frame_ms"] = frames
		results.append(row)
	...
	var report := {"display_name": DisplayServer.get_name(), "rendering_method": RenderingServer.get_current_rendering_method(), "window_size": [root.size.x, root.size.y], "vsync_mode_observed": DisplayServer.window_get_vsync_mode() if graphical else -1, "engine_max_fps_observed": Engine.max_fps, "platform": OS.get_name(), "cpu": OS.get_processor_name(), "godot": Engine.get_version_info()["string"], "renderer": DisplayServer.get_name(), "warmup_ticks": WARMUP, "temporary_profile": true, "passed_execution": not failed, "cases": results}
