...
var _movement_carrier: ProductionMovementIntentCarrier
var _movement_context: PcMovementContext
var movement_consume_count := 0

## Binds the GameRoot-created PC storage for this player instance.
func bind_movement(carrier: ProductionMovementIntentCarrier, context: PcMovementContext) -> void:
	_movement_carrier = carrier
	_movement_context = context

## Rejects foreign, stale or already consumed input before any player mutation.
func consume_movement(carrier: ProductionMovementIntentCarrier, context: PcMovementContext,
		lease: int, delta: float) -> bool:
	if carrier != _movement_carrier or context != _movement_context or context == null \
			or not context.open or context.retired or context.consumed or lease != context.lease_id \
			or carrier.written_tick != context.tick or not carrier.direction.is_finite():
		return false
	if carrier.is_active:
		if carrier.press_generation < 1 or not is_equal_approx(carrier.direction.length(), 1.0):
			return false
	elif carrier.direction != Vector2.ZERO or carrier.press_generation != -1:
		return false
	if not is_finite(delta) or delta < 0.0:
		return false
	if not run_phase(&"PLAYER_MOVE", carrier.direction, delta):
		return false
	context.consumed = true
	movement_consume_count += 1
	return true
