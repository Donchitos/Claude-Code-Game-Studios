...
func _physics_process(delta: float) -> void:
	if state != State.BATTLE_ACTIVE or current_battle == null or _transition_pending or not window_focused:
		return
	physics_tick += 1
...
func _input(event: InputEvent) -> void:
	...
	if action != &"" and window_focused and not viewport_gate_held:
		get_viewport().set_input_as_handled()
...
func request_resume() -> Status:
	if state != State.BATTLE_PAUSED or current_battle == null or current_battle.pending_upgrade or not window_focused:
		return Status.WRONG_STATE
	var resume_revision := focus_revision
	state = State.RESUME_PREPARING
	if acquire_viewport_input_gate() != Status.OK:
		return _enter_fault("RESUME_GATE_ACQUIRE_FAILED")
	get_tree().paused = false
	if not window_focused or focus_revision != resume_revision:
		get_tree().paused = true
		state = State.BATTLE_PAUSED
		release_viewport_input_gate(ACTIVATION_SUCCESS)
		return Status.WRONG_STATE
