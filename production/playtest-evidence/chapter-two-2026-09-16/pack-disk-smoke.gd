...
func _run():
	...
	root.get_texture().get_image().save_png("/Users/tal/CursorProjects/xiuxian/Claude-Code-Game-Studios/production/playtest-evidence/chapter-two-2026-09-16/pack-disk-smoke.png")
	game.queue_free()
	await process_frame
	await process_frame
	assert(DirAccess.remove_absolute(slot_a) == OK)
	assert(DirAccess.remove_absolute(slot_b) == OK)
	print("G_PACK_DISK_SMOKE_PASS 120 ticks saved and reloaded; isolated test slots removed ",OS.get_user_data_dir())
	quit()
