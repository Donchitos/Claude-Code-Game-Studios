...
func _run():
	...
	var input=JSON.parse_string(FileAccess.get_file_as_string(OS.get_cmdline_user_args()[0]))
	var a=Arena.new();root.add_child(a)
	var m=c.missions[int(input.index)]
	assert(a.configure(c,m,input.loadout,int(input.seed)))
	for tick in int(float(m.timeout_seconds)*60)+120:
		if a.state.finished: break
		if not a.state.offered.is_empty():assert(a.choose_upgrade(_bot_choice(a,c)))
		elif a.state.event_id!="":assert(a.choose_event(false))
		a.advance(1.0/60,_bot_direction(a,m,tick))
	print("LATER_CHAPTER_OBSERVATION ",JSON.stringify({"catalog":c.content_hash,"victory":a.state.victory,"reason":a.state.reason,"elapsed":a.state.elapsed,"stats":a.stats(),"hp":a.state.player.hp}))
	a.free()
	quit()
...
func _bot_direction(a, m: Dictionary, tick_value: int) -> Vector2:
	...
	assert(not m.has("clues")) # This probe only compares the undecorated sixth-chapter mission.
