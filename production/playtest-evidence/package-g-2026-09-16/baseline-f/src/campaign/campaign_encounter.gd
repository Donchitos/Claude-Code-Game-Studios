...
static func valid_definition(c: Dictionary, m: Dictionary) -> bool:
	...
	if not m.get("chapter_c",false):
		for key in ["clues","clue_radius","hunt_enemy_id","hunt_retry_ticks","hunt_charge","event_ids","close_roots","boss_chapter"]:
...
static func advance(a) -> void:
	...
	for root_index in roots.size():
		...
		var cycle := int(row.warning_ticks)+int(row.active_ticks)+int(row.rest_ticks)
		if not a.state.objective.finished and a.state.tick > row.offset_ticks and (int(a.state.tick)-int(row.offset_ticks)-1)%cycle == 0:
