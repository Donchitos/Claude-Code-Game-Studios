...
```
Delegate a bounded review to a generic agent using
`roles/creative-director.md` and gate **CD-PILLARS** from
...
**Check pattern - apply before every gate delegation:**

```
Before delegating gate review [GATE-ID]:
...
- full -> delegate the review as normal
```
...
**MANDATORY: Resolve review mode before every gate delegation.** Never delegate a gate without checking. The resolved mode is determined once per skill run:
1. If skill was called with `--review [mode]`, use that
2. Else read `production/review-mode.txt`
3. Else default to `lean`
...
Delegate to a generic agent using `roles/[profile-name].md`:
- Gate: [GATE-ID] (see docs/studio/director-gates.md)
- Context: [fields listed under that gate]
- Await the verdict before proceeding.
```

For multiple independent reviews at the same gate point:

```
# Apply the mode check for each gate first, then delegate all independent reviews.
Run them concurrently when the Codex client supports safe parallel delegation;
otherwise run them serially. Collect every verdict before proceeding.
```
...
| **CONCERNS [list]** | Issues present but not blocking. | Present options to the user: `Revise flagged items` / `Accept and proceed` / `Discuss further` |
...
**Escalation rule**: When multiple director reviews are delegated, apply the
...
Role profile: `creative-director` | Domain: Vision, pillars, player experience
...
**Trigger**: Always at `$gate-check` - delegate alongside TD-PHASE-GATE and PR-PHASE-GATE
...
Role profile: `technical-director` | Domain: Architecture, engine risk, performance
...
**Trigger**: Always at `$gate-check` - delegate alongside CD-PHASE-GATE and PR-PHASE-GATE
...
Role profile: `producer` | Domain: Scope, timeline, dependencies, production risk
...
**Trigger**: Always at `$gate-check` - delegate alongside CD-PHASE-GATE and TD-PHASE-GATE
...
Role profile: `art-director` | Domain: Visual identity, art bible, visual production readiness
...
**Trigger**: Always at `$gate-check` - delegate alongside CD-PHASE-GATE, TD-PHASE-GATE, and PR-PHASE-GATE
...
specialist's feasibility sign-off is needed. The coordinating Codex session
selects the model and supplies the relevant lead profile.
...
at `$gate-check`), delegate every independent review before reconciling results:

```
Delegate concurrently when supported, otherwise serially:
1. `roles/creative-director.md`  -> gate CD-PHASE-GATE
2. `roles/technical-director.md` -> gate TD-PHASE-GATE
3. `roles/producer.md`           -> gate PR-PHASE-GATE
4. `roles/art-director.md`       -> gate AD-PHASE-GATE
