...
static func boss(a, e: Dictionary, target: Vector2) -> void:
	...
	if phase > int(c.boss_phase) and a.mission.get("boss_phase_entry_warning",false): e.timer = 0
	c.boss_phase = phase
	e.phase = phase
	...
	# Preserve published landings; entry warning starts after they drain.
	if e.timer > 0 or (a.mission.get("boss_phase_entry_warning",false) and not c.landings.is_empty()): return
