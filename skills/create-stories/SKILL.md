...
- Canonical syntax: `$create-stories`
- Expected input: `[epic-slug | epic-path] [--review full|lean|solo]`. Input is the text supplied by the user's invoking prompt.

## Role

Adopt the [lead-programmer](../../roles/lead-programmer.md) profile locally when running this skill, or inject that profile into the prompt for a generic Codex sub-agent. The profile provides repository guidance; it is not a registered custom agent type.
...
- No argument — ask: "Which epic would you like to break into stories?"
  Enumerate `production/epics/*/EPIC.md` and list available epics with their status.
...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `qa-lead` through generic sub-agent delegation using gate **QL-STORY-READY** (`docs/studio/director-gates.md`).
...
**Before generating test specs**: Find the most recently modified `production/qa/qa-plan-*.md` file. If found, read it and check whether it contains test case specifications for the stories in this epic (look for story titles or slugs in the plan's Automated Tests Required section). If matching specs exist:
...
Pause and ask the user a focused question to close with context-aware next steps:
