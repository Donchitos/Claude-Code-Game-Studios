...
- Canonical syntax: `$art-bible`
- Expected input: `[--review full|lean|solo]`. Input is the text supplied by the user's invoking prompt.
...
**Retrofit mode detection**: Check for `design/art/art-bible.md`. If the file exists:
- Read it in full
...
Pause and ask the user these two focused questions together:
...
**Agent delegation (MANDATORY)**: Spawn `art-director` through generic sub-agent delegation:
...
Present the art-director's draft to the user. Pause and ask the user a focused question:
...
Collect both. If they conflict (e.g., art-director wants elaborate diegetic UI but ux-designer flags it would reduce combat readability), surface the conflict explicitly with both positions. Do NOT silently resolve — pause and ask the user a focused question to let the user decide.
...
After all sections are complete (or the scoped set from Phase 1 is complete), spawn `creative-director` through generic sub-agent delegation using gate **AD-ART-BIBLE** (`docs/studio/director-gates.md`).
...
Pause and ask the user a focused question for next steps. Only include options that are genuinely next based on the state check above:
