...
- Canonical syntax: `$start`
- Expected input: None. Invoke the skill without additional input.
...
- **Source code exists?** Enumerate source files in `src/` (`*.gd`, `*.cs`, `*.cpp`, `*.h`, `*.rs`, `*.py`, `*.js`, `*.ts`).
...
This is the first thing the user sees. Pause and ask the user a focused question with these exact options so the user can click rather than type:
...
1. Ask them to describe their concept in one sentence — genre and core mechanic. Use plain text, not a focused user question (it's an open response).
2. Acknowledge the concept, then pause and ask the user a focused question to offer two paths:
...
After presenting the recommended path, pause and ask which step the user would like to take first. Never auto-run the next skill.
