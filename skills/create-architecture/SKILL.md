...
- Canonical syntax: `$create-architecture`
- Expected input: `[focus-area: full | layers | data-flow | api-boundaries | adr-audit] [--review full|lean|solo]`. Input is the text supplied by the user's invoking prompt.
...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then pause and ask the user a focused question:
...
**Step 2 — Spawn `lead-programmer` through generic sub-agent delegation using gate LP-FEASIBILITY (`docs/studio/director-gates.md`):**
...
Pause and ask the user a focused question — "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
Show the proposed Document Status block inline, then pause and ask the user a focused question:
