...
- Canonical syntax: `$skill-improve`
- Expected input: `[skill-name]`. Input is the text supplied by the user's invoking prompt.
...
Failing: Check 3 (Codex metadata), Check 7 (no handoff)
```
...
- **Check 2 fail** → skill directory and frontmatter names differ
- **Check 3 fail** → missing or incomplete `agents/openai.yaml` interface metadata
- **Check 4 fail** → how many phases found vs. minimum required
- **Check 5 fail** → no verdict keywords anywhere in the skill body
- **Check 6 warn** → no ask-before-write language
- **Check 7 warn** → no follow-up or next-step section at the end
