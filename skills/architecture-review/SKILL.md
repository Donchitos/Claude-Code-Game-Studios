...
- Canonical syntax: `$architecture-review`
- Expected input: `[focus: full | coverage | consistency | engine | rtm | single-gdd path/to/gdd.md]`. Input is the text supplied by the user's invoking prompt.

## Role

Adopt the [technical-director](../../roles/technical-director.md) profile locally when running this skill, or inject that profile into the prompt for a generic Codex sub-agent. The profile provides repository guidance; it is not a registered custom agent type.

## Reasoning level

Use deep reasoning for this cross-document or high-stakes review workflow.
...
Before reading any full document, use `rg` to extract `## Summary` sections
from all GDDs and ADRs:

```
rg -n -A 4 "^## Summary" design/gdd -g "*.md"
rg -n -A 3 "^## Summary" docs/architecture -g "adr-*.md"
```

For `single-gdd [path]` mode: use the target GDD's summary to identify which
ADRs reference the same system (search ADRs with `rg` for the system name), then full-read
...
Enumerate `production/epics/**/*.md` (excluding EPIC.md index files). For each
story file:
...
Enumerate `tests/unit/**/*_test.*` and `tests/integration/**/*_test.*`.
Build an index: system → [test file paths].

For each test file path from Step 3b-1, confirm whether the file
...
- Search all ADRs with `rg` for API names listed in `deprecated-apis.md`
...
After completing the engine audit above, spawn the **primary engine specialist** through generic sub-agent delegation for a domain-expert second opinion:
...
- Spawn configured primary specialist role profile with: all ADRs that contain engine-specific decisions or `Post-Cutoff APIs Used` fields, the engine reference docs, and the Phase 5 audit findings. Ask them to:
...
If [B]: display the complete proposed systems-index section, then re-ask with a focused user question.
...
Pause and ask the user a focused question for the write approval:
...
For `rtm` mode, pause and ask the user a focused question:
...
Then close with a focused user question tailored to the pre-gate checklist state:
...
3. **Offer options** via a focused user question with three choices:
...
5. **Pause and ask the user a focused question for write approvals** — plain text "May I?" is not
