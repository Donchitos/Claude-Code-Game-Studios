...
- Canonical syntax: `$milestone-review`
- Expected input: `[milestone-name|current] [--review full|lean|solo]`. Input is the text supplied by the user's invoking prompt.
...
Before generating the Go/No-Go recommendation, spawn `producer` through generic sub-agent delegation using gate **PR-MILESTONE** (`docs/studio/director-gates.md`).
...
If OFF TRACK, pause and ask the user a focused question before generating the recommendation:
...
If AT RISK, pause and ask the user a focused question:
