...
- Canonical syntax: `$adopt`
- Expected input: `[focus: full | gdds | adrs | stories | infra]`. Input is the text supplied by the user's invoking prompt.
- Missing input: Default to `full` and audit all artifact types.
...
**Audit mode:** the invocation input (blank = `full`)
...
- Check `docs/adoption-plan-*.md` — note the filename of the most recent prior plan if any exist
...
If the project appears fresh (no artifacts at all), pause and ask the user a focused question:
...
1. **Parenthetical status values** — Search with `rg` for any Status cell containing
...
**If it does not exist**: Pause and ask the user a focused question:
...
and offer to handle it immediately by pausing and asking the user a focused question. Choose the first
branch that applies:
