...
- Canonical syntax: `$bug-report`
- Expected input: `[description] | analyze [path-to-file]`. Input is the text supplied by the user's invoking prompt.
...
2. **Search the codebase** for related files using `rg` and `rg --files` to add context (affected system, likely files).
...
1. **Re-run reproduction steps** — use `rg` and `rg --files` to check whether the root cause code path still exists as described. If the fix removed or changed it, note the change.
