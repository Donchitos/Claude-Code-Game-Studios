...
- Canonical syntax: `$day-one-patch`
- Expected input: `[scope: known-bugs | cert-feedback | all]`. Input is the text supplied by the user's invoking prompt.
...
Pause and ask the user a focused question to confirm proceeding or reduce scope.
...
Spawn `release-manager` through generic sub-agent delegation. Ask them to produce a rollback plan covering:
...
1. Spawn `lead-programmer` through generic sub-agent delegation with:
...
3. Spawn `qa-tester` through generic sub-agent delegation to verify: does the bug reproduce after the fix?
...
Spawn `qa-lead` through generic sub-agent delegation with:
