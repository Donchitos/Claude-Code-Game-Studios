...
- Canonical syntax: `$security-audit`
- Expected input: `[full | network | save | input | quick]`. Input is the text supplied by the user's invoking prompt.

## Role

Adopt the [security-engineer](../../roles/security-engineer.md) profile locally when running this skill, or inject that profile into the prompt for a generic Codex sub-agent. The profile provides repository guidance; it is not a registered custom agent type.
...
Spawn `security-engineer` through generic sub-agent delegation. Pass:
...
Search with `rg` for `File.open`, `load`, `deserialize`, `JSON.parse`, `from_json`, and `read_file` — check each for validation.
...
Search with `rg` for `recv`, `receive`, `PacketPeer`, `socket`, `NetworkedMultiplayerPeer`, `rpc`, and `rpc_id` — check each call site for validation.
...
Search with `rg` for `get_input`, `Input.get_`, `input_map`, and user-facing text fields — check validation.
...
Search with `rg` for `api_key`, `secret`, `password`, `token`, `private_key`, `DEBUG`, and `print(` in release-facing code.
...
Check `addons/`, `plugins/`, `third_party/`, and `vendor/` — list all external dependencies.
