...
- Canonical syntax: `$review-all-gdds`
- Expected input: `[focus: full | consistency | design-theory | since-last-review]`. Input is the text supplied by the user's invoking prompt.
- Missing input: Default to `full` and run both consistency and design-theory passes.
...
Use generic Codex sub-agents; repository role profiles do not register custom agent types. Instruct each sub-agent to read its linked profile before acting, and include the full task context: complete scope, relevant paths, constraints, assigned checklist, and expected output.

- [systems-designer](../../roles/systems-designer.md) — Phase 2 cross-GDD consistency
- [game-designer](../../roles/game-designer.md) — Phase 3 design-theory review
...
**Focus:** the invocation input (blank = `full`)
...
```
rg -n -A 5 "^## Summary" design/gdd -g "*.md"
```
...
authoritative values and source GDDs. In Phase 2, search GDDs with `rg` for registered
...
the same GDD inputs but produce separate reports. Spawn two generic Codex
sub-agents simultaneously: inject the `systems-designer` profile for Phase 2
and the `game-designer` profile for Phase 3. Collect both results before writing
the combined report.

**When delegating Phase 2 and Phase 3, always pass:**
...
Do not assume a sub-agent inherits Phase 1 conversation context. Give it the explicit task context and file paths above; include already-loaded registry content when that content is required verbatim.
...
Pause and ask the user a focused question for write permission:
- Prompt: "May I write this review to `design/gdd/gdd-cross-review-[date].md`?"
- Options: `[A] Yes — write the report` / `[B] No — skip`

If any GDDs are flagged for revision, use a second a focused user question:
...
After all file writes are complete, pause and ask the user a focused question for a closing prompt.
