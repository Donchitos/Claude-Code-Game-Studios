...
- Canonical syntax: `$design-system`
- Expected input: `<system-name> [--review full|lean|solo]`. Input is the text supplied by the user's invoking prompt.
...
- [ai-programmer](../../roles/ai-programmer.md)
- [art-director](../../roles/art-director.md)
- [audio-director](../../roles/audio-director.md)
- [creative-director](../../roles/creative-director.md)
- [economy-designer](../../roles/economy-designer.md)
- [engine-programmer](../../roles/engine-programmer.md)
- [game-designer](../../roles/game-designer.md)
- [gameplay-programmer](../../roles/gameplay-programmer.md)
- [level-designer](../../roles/level-designer.md)
- [narrative-director](../../roles/narrative-director.md)
- [qa-lead](../../roles/qa-lead.md)
- [sound-designer](../../roles/sound-designer.md)
- [systems-designer](../../roles/systems-designer.md)
- [technical-artist](../../roles/technical-artist.md)
- [ui-programmer](../../roles/ui-programmer.md)
- [ux-designer](../../roles/ux-designer.md)
- [world-builder](../../roles/world-builder.md)
...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and pause and ask the user a focused question:
...
6. If yes: proceed to **Phase 2 (Gather Context)** as normal, but in **Phase 3**
   ...
   incomplete sections.
7. **Never overwrite existing section content.** Replace only
...
- **Related GDDs**: Enumerate `design/gdd/*.md` and read any that are thematically related
...
- Enumerate `docs/architecture/adr-*.md` and read any ADRs whose domain matches
...
Pause and ask the user a focused question:
...
- If it **does not exist**: create it. Never attempt an in-place edit on a file that may not exist.
- If it **already exists**: edit the relevant fields in place.
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   a focused user question for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then pause and ask the user a focused question to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   a focused user question. **NEVER use plain text. NEVER skip this step.**
...
7. **Write**: Replace the placeholder with the approved content.
   **CRITICAL**: Always include the section heading in the `old_string` to ensure
   ...
   same placeholder and the edit must target a unique match. Use this pattern:
   ```
...
completed section name. Check if the file exists — create it when missing
...
**Derive recommended options before building the prompt**: Read the system's category and layer from the systems index (already in context from Phase 2), then determine the recommended option for each question:
- **Framing question**: Foundation/Infrastructure layer → `[A]` recommended. Player-facing categories (Combat, UI, Dialogue, Character, Animation, Visual Effects, Audio) → `[C] Both` recommended.
- **ADR ref question**: Enumerate `docs/architecture/adr-*.md` and search with `rg` for the system name in each ADR's GDD Requirements section. If a matching ADR is found → `[A] Yes — cite the ADR` recommended. If none found → `[B] No` recommended.
- **Fantasy question**: Foundation/Infrastructure layer → `[B] No` recommended. All other categories → `[A] Yes` recommended.

Append `(Recommended)` to the appropriate option text in each question.

**Framing questions (ask BEFORE drafting)**: Pause and ask the user a focused question with a grouped prompt:
...
- **ADR ref** — "Should the overview reference the existing ADR for this system?" Options: `[A] Yes — cite the ADR for implementation details` / `[B] No — keep the GDD at pure design level`
...
**Framing question (ask BEFORE drafting)**: Pause and ask the user a focused question:
...
spawn `creative-director` through generic sub-agent delegation:
...
**Agent delegation (MANDATORY)**: Before drafting Section C, spawn specialist agents through generic sub-agent delegation in parallel:
...
- Surface any disagreements between agents to the user by pausing and asking the user a focused question
...
**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, spawn specialist agents through generic sub-agent delegation in parallel:
...
- Present the specialists' proposals to the user for review by pausing and asking the user a focused question
...
Before finalizing the GDD, spawn `creative-director` through generic sub-agent delegation using gate **CD-GDD-ALIGN** (`docs/studio/director-gates.md`).
...
```
rg -n "  - name: [candidate_name]" design/registry/entities.yaml
```
...
**When delegating via generic sub-agent mechanism**:
...
- The main session presents the agent's output to the user by pausing and asking the user a focused question
- The user decides; the main session writes to file
...
2. **Focused user questions** at every decision point (Explain -> Capture pattern):
