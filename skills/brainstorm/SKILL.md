...
- Canonical syntax: `$brainstorm`
- Expected input: `[genre or theme hint, or 'open'] [--review full|lean|solo]`. Input is the text supplied by the user's invoking prompt.
...
- [technical-director](../../roles/technical-director.md)
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Pause and ask the user a focused question** at key decision points throughout brainstorming:
   ...
   a focused user question to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in a focused user question with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Pause and ask the user a focused question for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into one grouped set of focused questions with these exact labels:
...
Use exactly these labels — do not rename or duplicate them.
...
Present all three. Then pause and ask the user a focused question to capture the selection.

**CRITICAL**: Ask one focused selection question. Do not combine it with unrelated questions. Use exactly this structure:
...
Keep this as one prompt with one option list so the user can make a single clear selection.
...
Ask these with focused user questions — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, pause and ask the user a focused question:
...
If the user selects B, C, or D, make the revision, then pause and ask the user a focused question again:
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` through generic sub-agent delegation in parallel before moving to Phase 5. Issue both generic sub-agent requests simultaneously — do not wait for one before starting the other.**
...
Collect both verdicts, then present two focused questions together:
- **Pillars**: present creative-director feedback. Options mirror the standard CD-PILLARS handling — `Lock in as-is` / `Revise [specific pillar]` / `Discuss further`.
- **Visual anchor**: present the art-director's 2-3 named visual direction options. Options: each named direction (one per option) + `Combine elements across directions` + `Describe my own direction`.
...
- **Target platform**: Pause and ask the user a focused question — "What platforms are you targeting for this game?"
...
- **Engine experience**: Pause and ask the user a focused question — "Do you already have an engine you work in?"
...
**After identifying biggest technical risks, spawn `technical-director` through generic sub-agent delegation using gate TD-FEASIBILITY (`docs/studio/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` through generic sub-agent delegation using gate PR-SCOPE (`docs/studio/director-gates.md`).**
...
5. Pause and ask the user a focused question for write approval:
...
If [B]: ask which section to revise by pausing and asking the user a focused question with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then pause and ask the user a focused question — "Ready to write the updated concept document?"
