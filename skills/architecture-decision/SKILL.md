...
- Canonical syntax: `$architecture-decision`
- Expected input: `[title] [--review full|lean|solo]`. Input is the text supplied by the user's invoking prompt.
...
5. If yes:
   ...
   - Append each missing section to the ADR file with a targeted edit.
...
a **confirm/adjust** prompt by pausing and asking the user a focused question — not open-ended questions.
...
**Scope of the assumptions question**: Assumptions cover only: problem framing, alternative approaches, upstream dependencies, GDD linkage, and status. Schema design questions (e.g., "How should spawn timing work?", "Should data be inline or external?") are NOT assumptions — they are design decisions belonging to a separate step after the assumptions are confirmed. Do not include schema design questions in the assumptions confirmation question.

**After assumptions are confirmed**, if the ADR involves schema or data design choices, use a separate grouped set of focused questions to ask each design question independently before drafting.

**Present assumptions with a focused user question:**
...
decisions remain, present each one as a separate focused user question with the proposed
...
5.5. **Engine Specialist Validation** — Before saving, spawn the **primary engine specialist** through generic sub-agent delegation to validate the drafted ADR:
   ...
   - Spawn configured primary specialist role profile with: the ADR's Engine Compatibility section, Decision section, Key Interfaces, and the engine reference docs path. Ask them to:
...
5.6. **Technical Director Strategic Review** — After the engine specialist validation, spawn `technical-director` through generic sub-agent delegation using gate **TD-ADR** (`docs/studio/director-gates.md`):
...
5. **Write approval** — Pause and ask the user a focused question:
...
Ask by pausing and asking the user a focused question:
...
After the ADR is written (and registry optionally updated), close with a focused user question.
