...
- Canonical syntax: `$prototype`
- Expected input: `[concept-description] [--path html|engine|paper] [--review full|lean|solo] [--spike]`. Input is the text supplied by the user's invoking prompt.

## Role

Adopt the [prototyper](../../roles/prototyper.md) profile locally when running this skill, or inject that profile into the prompt for a generic Codex sub-agent. The profile provides repository guidance; it is not a registered custom agent type.

## Compatibility

Legacy metadata requested worktree isolation. Codex skill metadata does not create or enforce a worktree. Before editing, explicitly create or select the intended Git worktree and verify its path and branch; pause for user direction if isolation is unavailable.
...
Otherwise, pause and ask the user a focused question to confirm intent before proceeding:
...
Assess which path best fits the hypothesis, then pause and ask the user a focused question with your
...
- `full` → spawn `creative-director` through generic sub-agent delegation using gate **CD-PLAYTEST** if
...
2. **Choose path** — use the same focused user question as Phase 3 (HTML / Engine / Paper).
