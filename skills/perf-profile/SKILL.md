...
- Canonical syntax: `$perf-profile`
- Expected input: `[system-name or 'full']`. Input is the text supplied by the user's invoking prompt.

## Role

Adopt the [performance-analyst](../../roles/performance-analyst.md) profile locally when running this skill, or inject that profile into the prompt for a generic Codex sub-agent. The profile provides repository guidance; it is not a registered custom agent type.
