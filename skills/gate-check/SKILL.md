...
- Canonical syntax: `$gate-check`
- Expected input: `[target-phase: systems-design | technical-setup | pre-production | production | polish | release] [--review full|lean|solo]`. Input is the text supplied by the user's invoking prompt.
- Missing input: Auto-detect the current stage, propose the next transition, and pause for confirmation.
...
**Target phase:** the invocation input (blank = auto-detect current stage, then validate next transition)
...
### Artifact Checks
- Verify that files exist and read them to confirm they have meaningful content
...
Find the `$review-all-gdds` report matching `design/gdd/gdd-cross-review-*.md`.
...
- For localization checks: use `rg` to find hardcoded strings in `src/`
...
Before generating the final verdict, spawn all four directors as **parallel subagents** through generic sub-agent delegation using the parallel gate protocol from `docs/studio/director-gates.md`. Issue all four generic sub-agent requests simultaneously — do not wait for one before starting the next.
...
> **Evidence-action requirement**: At least 2 of the 5 challenge questions below must be answered by re-reading a specific file or re-running a concrete check with `rg` or the relevant project command — not by reflection alone. Mark these with [EVIDENCE ACTION].
...
## 7. Closing Next-Step Question

After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt by pausing and asking the user a focused question.
