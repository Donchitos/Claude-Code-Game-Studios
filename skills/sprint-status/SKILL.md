...
- Canonical syntax: `$sprint-status`
- Expected input: `[sprint-number or blank for current]`. Input is the text supplied by the user's invoking prompt.
- Missing input: Use the most recently modified sprint plan as the current sprint.
...
**Argument:** the invocation input (blank = use current sprint)
