...
- Canonical syntax: `$asset-spec`
- Expected input: `[system:<name> | level:<name> | character:<name>] [--review full|lean|solo]`. Input is the text supplied by the user's invoking prompt.
...
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and pause and ask the user a focused question:
...
Present the full proposed inventory to the user in conversation. Then pause and ask the user a focused question:
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Pause and ask the user a focused question: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, pause and ask the user a focused question:
...
- **Related specs**: Check `design/assets/specs/*.md` — scan for assets that could be shared (e.g., a common UI element specced for one system might apply here too).
...
Present the full identified list to the user. Pause and ask the user a focused question:
...
Spawn specialist agents based on review mode. **Issue all generic sub-agent requests simultaneously — do not wait for one before starting the next.**

### Full mode — spawn in parallel:

**`art-director`** through generic sub-agent delegation:
...
**`technical-artist`** through generic sub-agent delegation:
...
After presenting all specs, pause and ask the user a focused question:
...
```
rg -n "ASSET-" design/assets/asset-manifest.md
```
