...
- Canonical syntax: `$map-systems`
- Expected input: `[next | system-name] [--review full|lean|solo]`. Input is the text supplied by the user's invoking prompt.
...
- Enumerate `design/gdd/*.md` — check which system GDDs already exist

**If the systems index already exists:**
- Read it and present current status to the user
- Pause and ask the user a focused question to ask:
...
Then pause and ask the user a focused question to capture feedback:
...
Pause and ask the user a focused question to ask: "Does this dependency ordering look right? Any
...
**After dependency mapping is approved, spawn `technical-director` through generic sub-agent delegation using gate TD-SYSTEM-BOUNDARY (`docs/studio/director-gates.md`) before proceeding to priority assignment.**
...
Pause and ask the user a focused question to ask: "Do these priority assignments match your vision?
...
**After priorities are approved, spawn `producer` through generic sub-agent delegation using gate PR-SCOPE (`docs/studio/director-gates.md`) before writing the index.**
...
**After the systems index is written, spawn `creative-director` through generic sub-agent delegation using gate CD-SYSTEMS (`docs/studio/director-gates.md`).**
...
Pause and ask the user a focused question for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `$design-system` completes, pause and ask the user a focused question:
...
After the systems index is created (or after designing some systems), present next actions by pausing and asking the user a focused question:
