...
- Canonical syntax: `$story-done`
- Expected input: `[story-file-path] [--review full|lean|solo]`. Input is the text supplied by the user's invoking prompt.
...
- [lead-programmer](../../roles/lead-programmer.md)
...
3. If multiple in-progress stories are found, pause and ask the user a focused question:
...
- **File existence check**: Check for files the story said would be created.
- **Test pass check**: if a test file path is mentioned, run it via `Bash`.
- **No hardcoded values check**: Search with `rg` for numeric literals in gameplay code
  paths that should be in config files.
- **No hardcoded strings check**: Search with `rg` for player-facing strings in `src/`
  that should be in localization files.
- **Dependency check**: if a criterion says "depends on X", check that X exists.

### Manual verification with confirmation (pause and ask the user a focused question)
...
Batch up to 4 manual verification questions into a single focused user question:
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **Unit test**: check `tests/unit/` for a test file or function name that
     matches the criterion's subject (use `rg --files` and `rg`)
   - **Integration test**: check `tests/integration/` similarly
   - **Manual confirmation**: if the criterion was verified by pausing and asking the user a focused question
...
exact required file path. Check that exact path. If the exact path is not
...
**For Visual/Feel and UI stories**: check `production/qa/evidence/` for a file
...
- If found: read the file and check the sign-off table for unchecked boxes. Use `rg` to find lines matching `| .* | .* | .* | \[ \] Approved` (a sign-off row with an unchecked checkbox). If any unchecked sign-off rows are found: flag as **ADVISORY** — "Evidence file found at `[path]` but [N] sign-off(s) are still pending (shown as `[ ] Approved` in the sign-off table). Obtain required sign-offs before final closure. Note: for solo developers, all roles may be signed off by the same person."
...
1. **GDD rules check**: Using the current requirement text from `tr-registry.yaml`
   ...
   Search the implemented files with `rg` for key function names, data structures, or class
...
3. **ADR constraints check**: Read the referenced ADR's Decision section. Check
   ...
   exists). Search with `rg` for patterns explicitly forbidden in the ADR.

4. **Hardcoded values check**: Search the implemented files with `rg` for numeric literals
...
After completing the deviation checks in Phase 4, spawn `qa-lead` through generic sub-agent delegation using gate **QL-TEST-COVERAGE** (`docs/studio/director-gates.md`).
...
- `lean` → pause and ask the user a focused question before proceeding:
...
Spawn `lead-programmer` through generic sub-agent delegation using gate **LP-CODE-REVIEW** (`docs/studio/director-gates.md`).

Pass: implementation file paths, story file path, relevant GDD section, governing ADR.

Present the verdict to the user. If CONCERNS, surface them by pausing and asking the user a focused question:
...
Pause and ask the user a focused question before writing anything:
...
- Pause and ask the user a focused question for the code review prompt and for batching manual
