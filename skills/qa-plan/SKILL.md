...
- Canonical syntax: `$qa-plan`
- Expected input: `[sprint | feature: system-name | story: path]`. Input is the text supplied by the user's invoking prompt.
- Missing input: Pause and ask the user to choose a sprint, feature, story, or epic scope.
...
**Argument:** the invocation input (blank = ask user via a focused user question)
...
- **`sprint`** — read the most recent file in `production/sprints/`, extract
  ...
  metadata.
- **`feature: [system-name]`** — enumerate `production/epics/*/story-*.md`, filter
...
- **No argument** — pause and ask the user a focused question:
...
then ask two questions together by pausing and asking the user a focused question:

```
question: "Ready to write the QA plan. Choose output options:"
allowing the user to select more than one option
options:
...
- Pause and ask the user a focused question for scope selection when no argument is provided.
