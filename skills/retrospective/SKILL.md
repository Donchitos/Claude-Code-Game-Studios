...
- Canonical syntax: `$retrospective`
- Expected input: `[sprint-N|milestone-name]`. Input is the text supplied by the user's invoking prompt.
...
Before loading any data, check for an existing retrospective file:
...
If a matching file is found, pause and ask the user a focused question:
...
Then pause and ask the user a focused question to present two options:
...
Run `git log` for the sprint period to understand what was actually committed and when. The following example uses Bash syntax (`2>/dev/null` is not PowerShell):
