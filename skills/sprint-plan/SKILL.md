...
- Canonical syntax: `$sprint-plan`
- Expected input: `[new|update|status] [--review full|lean|solo]`. Input is the text supplied by the user's invoking prompt.

## Compatibility

Legacy metadata preloaded the sprint directory. Codex skill metadata does not run preload commands, so explicitly inspect `production/sprints/` before resolving the mode.
...
- If the file doesn't exist and this is a `new` sprint: pause and ask the user a focused question:
...
3. Pause and ask the user which stories to add, remove, reprioritize, or re-estimate.
...
Before finalising the sprint plan, spawn `producer` through generic sub-agent delegation using gate **PR-SPRINT** (`docs/studio/director-gates.md`).
...
Look for `production/qa/qa-plan-sprint-[N].md` or any file in `production/qa/` referencing this sprint number.
