...
- Canonical syntax: `$test-evidence-review`
- Expected input: `[story-path | sprint | system-name]`. Input is the text supplied by the user's invoking prompt.
...
**System**: Enumerate `production/epics/[system-name]/story-*.md`. Read each.
...
**Logic stories**: Check `tests/unit/[system]/[story-slug]_test.*`
  - If not found, also search `tests/unit/[system]/` with `rg` for files
    containing the story slug

**Integration stories**: Check `tests/integration/[system]/[story-slug]_test.*`
  - Also check `production/session-logs/` for playtest records mentioning the story

**Visual/Feel and UI stories**: Check `production/qa/evidence/[story-slug]-evidence.*`

**Config/Data stories**: Check `production/qa/smoke-*.md` (any smoke check report)
...
Heuristics:
- Search the test file with `rg` for "zero", "max", "null", "empty", "min", "invalid",
...
in the evidence doc. If referenced, confirm that they exist.
