...
- Canonical syntax: `$dev-story`
- Expected input: `[story-path]`. Input is the text supplied by the user's invoking prompt.
...
If not found, ask: "Which story are we implementing?" Search
...
If they differ, pause and ask the user a focused question before proceeding:
...
1. Enumerate `production/epics/**/*.md` to find each dependency story file.
...
specialist to spawn through generic sub-agent delegation.
...
Spawn the chosen programmer agent(s) through generic sub-agent delegation with the full context package:

Brief the agent with file paths and targeted reading instructions — do not serialize document content into the sub-agent prompt. The agent reads what it needs directly:
...
- **File writes are delegated** — all source code, test files, and evidence docs are written by sub-agents spawned through generic sub-agent delegation. Each sub-agent enforces the "May I write to [path]?" protocol individually. This orchestrator does not write files directly.
