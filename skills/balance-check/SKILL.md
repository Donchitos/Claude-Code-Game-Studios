...
- Canonical syntax: `$balance-check`
- Expected input: `[system-name|path-to-data-file]`. Input is the text supplied by the user's invoking prompt.
- Missing input: Pause and ask the user which system or data file to analyze.

## Role

Adopt the [economy-designer](../../roles/economy-designer.md) profile locally when running this skill, or inject that profile into the prompt for a generic Codex sub-agent. The profile provides repository guidance; it is not a registered custom agent type.
...
Determine the balance domain from the invocation input:
...
After presenting the report, pause and ask the user a focused question:
