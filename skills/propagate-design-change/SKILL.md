...
- Canonical syntax: `$propagate-design-change`
- Expected input: `[path/to/changed-gdd.md]`. Input is the text supplied by the user's invoking prompt.
...
Spawn `technical-director` through generic sub-agent delegation using gate **TD-CHANGE-IMPACT** (`docs/studio/director-gates.md`).
...
- **CONCERNS** → surface the specific ADRs or recommendations flagged; pause and ask the user a focused question with options: `Revise the impact assessment` / `Accept with noted concerns` / `Discuss further`
