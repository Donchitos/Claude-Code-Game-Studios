...
- Canonical syntax: `$smoke-check`
- Expected input: `[sprint | quick | --platform pc|console|mobile|all]`. Input is the text supplied by the user's invoking prompt.
...
2. Enumerate files in `tests/unit/[system]/` and `tests/integration/[system]/`
...
Pause and ask the user a focused question to batch-verify. Keep to at most 3 calls.
...
- Pause and ask the user a focused question for all manual smoke check verification.
