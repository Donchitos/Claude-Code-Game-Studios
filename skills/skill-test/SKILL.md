...
- Canonical syntax: `$skill-test`
- Expected input: `static [skill-name | all] | spec [skill-name] | role [role-name] | category [skill-name | all] | audit`. Input is the text supplied by the user's invoking prompt.
...
| `role` | `$skill-test role [name]` | Role-profile verifier — evaluates a role spec | Medium (~5k/role) |
...
- `static all` → run 7 structural checks on all skills (`skills/*/SKILL.md`)
- `spec [name]` → read skill + test spec, evaluate assertions
- `role [name]` → read role profile + role spec, evaluate assertions
...
### Check 2 — Skill Directory Name
The frontmatter `name` must exactly match the skill directory name.

**FAIL** if the names differ.

### Check 3 — Codex Interface Metadata
The skill must include `agents/openai.yaml` with non-empty:
- `interface.display_name`
- `interface.short_description`

**FAIL** if the metadata file or either required interface field is absent.

### Check 4 — Multiple Phases
...
### Check 6 — Collaborative Protocol Language
...
Check 2 — Directory Name:        PASS
Check 3 — Codex Metadata:        PASS
Check 4 — Multiple Phases:       PASS (7 phases found)
Check 5 — Verdict Keywords:      PASS (PASS, FAIL, CONCERNS)
Check 6 — Collaborative Protocol: PASS ("May I write" found)
Check 7 — Next-Step Handoff:     WARN (no follow-up section found)
...
## Phase 2C: Role Profile Mode — Behavioral Verifier

Find the role profile at `roles/[name].md` and the matching entry under
`agents:` in `CCGS Skill Testing Framework/catalog.yaml`. Read the role profile
and its spec completely, then evaluate each test case against the profile's
documented domain, boundaries, escalation behavior, and collaboration protocol.

Do not require a fixed model, custom-agent registration, or legacy gate-token
format unless the current role profile explicitly documents it. Report each case
as PASS, PARTIAL, or FAIL and offer to record `last_spec` and
`last_spec_result` in the role's catalog entry after user approval.
...
Enumerate `skills/*/SKILL.md` to get the complete list of skills.
...
SKILLS (73 total)
Specs written: 73 (100%) | Never static tested: 73 | Never category tested: 73
