...
- Canonical syntax: `$test-flakiness`
- Expected input: `[ci-log-path | scan | registry]`. Input is the text supplied by the user's invoking prompt.
...
For Unreal projects: automation logs go to `Saved/Logs/`. Search with `rg` for
...
- Search with `rg` for `<testcase name=` to get test names
- Search with `rg` for `<failure` or `<error` to identify failures
- Parse `classname` and `name` attributes for full test identifiers

**Plain text logs**:
- Search with `rg` for pass/fail patterns:
...
Use `rg` to check the test file for timing calls, randf, global state access,
