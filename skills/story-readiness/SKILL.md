...
- Canonical syntax: `$story-readiness`
- Expected input: `[story-file-path or 'all' or 'sprint']`. Input is the text supplied by the user's invoking prompt.
- Missing input: Pause and ask whether to validate one story, the current sprint, or all stories.
...
**Scope:** the invocation input (blank = ask user via a focused user question)
...
If no argument is given, pause and ask the user a focused question:
...
- [ ] **Referenced assets exist**: Scan the story text for asset path patterns
  ...
  - For each asset path found: check whether the file exists.
...
in conversation. Do not edit files — the user (or
...
Spawn `qa-lead` through generic sub-agent delegation using gate **QL-STORY-READY** (`docs/studio/director-gates.md`).
...
- **GAPS [list]** → surface the specific gaps to the user by pausing and asking the user a focused question:
