...
- Canonical syntax: `$design-review`
- Expected input: `[path-to-design-doc] [--depth full|lean|solo]`. Input is the text supplied by the user's invoking prompt.
...
- [network-programmer](../../roles/network-programmer.md)
- [performance-analyst](../../roles/performance-analyst.md)
...
**Dependency graph validation:** For every system listed in the Dependencies section, check whether its GDD file exists in `design/gdd/`. Flag any that don't exist yet — these are broken references that downstream authors will hit.
...
**CRITICAL: Delegation in this skill uses a generic Codex sub-agent with its own
context window. It is NOT task tracking. Do NOT simulate specialist
perspectives internally. Do NOT reason through domain views yourself. You MUST issue
actual generic sub-agent requests. A simulated review is not a specialist review.**

Issue all generic sub-agent requests simultaneously. Do NOT spawn one at a time.
...
Pause and ask the user a focused question for ALL closing interactions. Never plain text.
...
Work through all blocking items, asking for design decisions only where you cannot resolve the issue from the GDD and existing docs alone. Group all design-decision questions into one focused user interaction before making any edits — do not interrupt mid-revision for each blocker individually.

After all revisions are complete, show a summary table (blocker → fix applied) and pause and ask the user a focused question for a **post-revision closing prompt**:

- Prompt: "Revisions complete — [N] blockers resolved. What next?"
- Note current context usage: if context is above ~50%, add: "(Recommended: start a new thread before re-review — this session has used X% context. A full re-review runs 5 agents and needs clean context.)"
- Options:
  - `[A] Re-review in a new session — run $design-review [doc-path] in the new thread`
...
When the verdict is APPROVED, use one focused user question that allows multiple selections to batch the two tracking updates:
...
Use a second a focused user question:
...
Use a third a focused user question:
...
Once the systems-index and review-log prompts are answered, check project state and ask one final focused user question:
