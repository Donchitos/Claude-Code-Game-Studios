...
- Canonical syntax: `$consistency-check`
- Expected input: `[full | since-last-review | entity:<name> | item:<name>]`. Input is the text supplied by the user's invoking prompt.
...
For each registered entry, search every in-scope GDD for the entry's name with `rg`.
...
(4,000 lines), you run 50 entity-name searches across 10 GDDs,
...
```
rg -n -C 3 "[entity_name]" design/gdd -g "*.md"
```
...
For each item in item_map, search all GDDs with `rg` for the item name. Extract:
...
For each formula in formula_map, search all GDDs with `rg` for the formula name. Extract:
...
For each constant in constant_map, search all GDDs with `rg` for the constant name. Extract:
...
```
Read `design/gdd/[conflicting_gdd].md`.
```
(Or use `rg -C <lines>` with wider context if the file is large.)
...
Then close with a focused user question:
