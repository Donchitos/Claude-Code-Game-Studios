...
- Canonical syntax: `$create-epics`
- Expected input: `[system-name | layer: foundation|core|feature|presentation | all] [--review full|lean|solo]`. Input is the text supplied by the user's invoking prompt.
...
Search all GDDs with `rg` for their `## Summary` sections before reading anything fully:
...
After all epics for the current layer are defined (Step 4 completed for all in-scope systems), and before writing any files, spawn `producer` through generic sub-agent delegation using gate **PR-EPIC** (`docs/studio/director-gates.md`).
...
If CONCERNS, pause and ask the user a focused question:
