...
- Canonical syntax: `$create-control-manifest`
- Expected input: `[update — regenerate from current ADRs]`. Input is the text supplied by the user's invoking prompt.
...
### ADRs
- Enumerate `docs/architecture/adr-*.md` and read every file
...
Spawn `technical-director` through generic sub-agent delegation using gate **TD-MANIFEST** (`docs/studio/director-gates.md`).
...
- **CONCERNS** → surface by pausing and asking the user a focused question with options: `Revise flagged rules` / `Accept and proceed` / `Discuss further`
