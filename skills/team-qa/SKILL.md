...
- Canonical syntax: `$team-qa`
- Expected input: `[sprint | feature: system-name] [--review full|lean|solo]`. Input is the text supplied by the user's invoking prompt.

## Role

Adopt the [qa-lead](../../roles/qa-lead.md) profile locally when running this skill, or inject that profile into the prompt for a generic Codex sub-agent. The profile provides repository guidance; it is not a registered custom agent type.
...
- [qa-lead](../../roles/qa-lead.md) role profile — Strategy, planning, classification, sign-off
- [qa-tester](../../roles/qa-tester.md) role profile — Test case writing and bug report writing
...
1. Detect the current sprint or feature scope from the argument:
   - If argument is a sprint identifier (e.g., `sprint-03`): Search `production/sprints/` for files matching `*[sprint-identifier]*.md`. Read the matched file. If multiple match, use the most recently modified.
...
Spawn `qa-lead` through generic sub-agent delegation to review all in-scope stories and produce a QA strategy.
...
Present the qa-lead's full strategy to the user, then pause and ask the user a focused question:
...
Spawn `qa-tester` through generic sub-agent delegation for each story (run in parallel where possible), providing:
- The story file path
...
Pause and ask the user a focused question per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and pause and ask the user a focused question for each:
...
After each FAIL result: pause and ask the user a focused question to collect the failure description, then spawn `qa-tester` through generic sub-agent delegation to write a formal bug report in `production/qa/bugs/`.
...
Spawn `qa-lead` through generic sub-agent delegation to produce the sign-off report using all results from Phases 4–6.
