...
- Canonical syntax: `$localize`
- Expected input: `[scan|extract|validate|status|brief|cultural-review|vo-pipeline|rtl-check|freeze|qa]`. Input is the text supplied by the user's invoking prompt.

## Role

Adopt the [localization-lead](../../roles/localization-lead.md) profile locally when running this skill, or inject that profile into the prompt for a generic Codex sub-agent. The profile provides repository guidance; it is not a registered custom agent type.
...
Spawn `localization-lead` through generic sub-agent delegation. Ask them to audit the following for cultural sensitivity across the target locales (read from `assets/data/strings/` and `assets/`):
...
Enumerate all `.wav`/`.ogg` files in `assets/audio/vo/[locale]/`. Cross-reference against the VO manifest. Report:
...
Search `src/` with `rg` for VO audio references. Verify each referenced path exists in `assets/audio/vo/[locale]/`. Report broken references.
...
Spawn `localization-lead` through generic sub-agent delegation with:
