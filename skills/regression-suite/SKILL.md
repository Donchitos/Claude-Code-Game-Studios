...
- Canonical syntax: `$regression-suite`
- Expected input: `[update | audit | report]`. Input is the text supplied by the user's invoking prompt.
...
- `$regression-suite report` — read-only status report (no writes); suitable
  for sprint reviews
- No argument — if a sprint is clearly active (sprint plan exists with in-progress stories), run `update`. If ambiguous or no active sprint is detected, pause and ask the user a focused question:
...
Enumerate `production/qa/bugs/*.md` and filter for bugs with a `Status: Closed`
...
1. Search `tests/unit/[system]/` and `tests/integration/[system]/` with `rg` for file names
...
2. Search `tests/unit/[system]/` and `tests/integration/[system]/` with `rg` for a test
...
- GDD sections added or revised since the regression suite was last updated
  (search GDD modification hints with `rg` if available, or ask the user)
