...
- Canonical syntax: `$code-review`
- Expected input: `[path-to-file-or-directory]`. Input is the text supplied by the user's invoking prompt.
...
Spawn all applicable specialists simultaneously through generic sub-agent delegation — do not wait for one before starting the next.
...
For Logic and Integration stories, also spawn `qa-tester` through generic sub-agent delegation in parallel with the engine specialists. Pass:
