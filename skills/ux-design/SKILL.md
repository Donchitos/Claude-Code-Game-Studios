...
- Canonical syntax: `$ux-design`
- Expected input: `[screen/flow name] or 'hud' or 'patterns'`. Input is the text supplied by the user's invoking prompt.

## Role

Adopt the [ux-designer](../../roles/ux-designer.md) profile locally when running this skill, or inject that profile into the prompt for a generic Codex sub-agent. The profile provides repository guidance; it is not a registered custom agent type.
...
**If no argument is provided**, do not fail — ask instead. Pause and ask the user a focused question:
...
Enumerate `design/gdd/*.md` and search with `rg` for `UI Requirements` sections. Read any GDD whose
...
Enumerate `design/ux/*.md` and note which screens already have specs. For screens that
...
Check `design/ux/[filename].md` (where `[filename]` is the resolved output path from Phase 1).
...
2. **Questions**: Ask what is needed to draft this section. Pause and ask the user a focused question
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then pause and ask the user a focused question to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Pause and ask the user a focused question:
...
7. **Write**: Pause and ask the user a focused question: "May I write the [section name] section to `[filepath]`?"
...
- Pause and ask the user a focused question to capture the choice:
...
- Pause and ask the user a focused question: "Want an ASCII wireframe as part of this spec?"
...
Pause and ask the user a focused question to confirm:
...
Pause and ask the user a focused question to step through items in groups of 3-4, not all at once.
...
Enumerate `design/ux/*.md` (excluding `interaction-patterns.md`) and read the Component
...
Work through patterns in groups. Pause and ask the user a focused question:
...
Then pause and ask the user a focused question:
...
When delegating to another agent via the generic sub-agent mechanism:
