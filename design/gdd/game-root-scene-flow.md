...
> **Status**: In Review — 首轮 full design-review（NEEDS REVISION）；5/7 修订批次闭环（RNG GATE-G3 集成 / TECHNICAL_ABORT 部分奖励 / R3 全量 copy / F1-F4 变量表 / AC-G1+AC-B1 守卫），Scoped A 删除 + AC 拆分待新会话
> **Author**: 用户 + Codex
> **Created**: 2026-08-19
> **Last Updated**: 2026-08-28 — 首轮 full design-review（6 specialist + CD），NEEDS REVISION，scope M；RNG/TECHNICAL_ABORT/R3/F1-F4/AC-G1/AC-B1 修订闭环，Scoped A + AC 拆分 pending
> **Implements Pillar**: 战斗—暂停选择—结算—再开局的可靠闭环
> **Scope**: MVP minimum；不定义各玩法系统内部规则
> **Review Mode**: full（首轮独立复审；此前 20 轮 InputSystem review 期间的增量为本 GDD 被 mirror 的契约）
...
- GameRoot 是战斗场景中唯一启用自主 physics orchestration 的节点。Enemy、Projectile、Drop、Damage、Player、Spawn 等 gameplay participant 不得定义或启用自己的`_physics_process/_process`、自行开启 SpatialGrid phase或依赖 SceneTree sibling 顺序；无状态表现更新如确有需要必须位于不参与gameplay phase的独立presentation adapter，不能复制GameRoot例外。
- 唯一GameRoot集中编排节点精确使用`PROCESS_MODE_ALWAYS`，这是项目与participant禁自主process规则的唯一显式例外。GameRoot恰定义两个引擎callback：`_physics_process(delta: float) -> void`只在SceneTree未暂停且处于允许技术tick的状态驱动唯一七phase physics orchestration；`_process(delta: float) -> void`只在`BATTLE_PAUSED/RESUME_PREPARING`驱动control pump。两个callback先检查权威state/pause predicate，未暂停时control pump调用数=0，Paused时七phase调用数=0，禁止双跑或互换职责。战斗phase participant（包括InputSystem participant）使用pausable mode；InputSystem拥有的`VirtualJoystickHost`、唯一typed `MovementIngressShield`与registered active VirtualJoystick是独立GUI adapter，精确使用`PROCESS_MODE_ALWAYS`。VJ只暴露`pressed()`与`released(Vector2)`，canceled ScreenTouch经非pressed路径归并为released；shield `_gui_input(InputEvent)`与Host `_notification(int)`是引擎虚回调，不依赖signal连接。adapter只能采集已arm的公开gesture/terminal状态，不能发布gameplay intent或推进GameRoot状态。BattleUI暂停/选择层可在pause下处理输入，但不得直接mutation SpatialGrid。
- GameRoot也是InputSystem顶层状态的唯一owner；只能在本GDD冻结的同步时点调用`publish_runtime_state(next_state,tick_revision)`。三字段职责与状态组合固定：`callbacks_armed`只覆盖initialize成功至teardown，`runtime_ingress_armed`只覆盖ACTIVE的新VJ press/generation/movement路径，`shield_bank_service_enabled`只覆盖LOCK_PENDING/FROZEN/RESUME_LOCKED的consumer-closed bank/FSM；IDLE=`true/false/false`、ACTIVE=`true/true/false`、三种locked state=`true/false/true`、TERMINATED=`false/false/false`。Host/shield callback只更新当前已启用service允许的预分配gesture/shield-bank/choice/error latch并可消费GUI event；仅lifecycle/geometry invalidation callback可对active node写一次预冻结的`mouse_filter=IGNORE`或经Godot 4.7.1证明的等价hit-route disable。`VirtualJoystickHost`唯一持有预创建typed `MovementIngressShield`。普通ACTIVE pause/fatal close由同步Input API在完成前置验证后，先以无函数/property setter/signal/Callable/`await`的主线程局部提交原子发布`Input state=LOCK_PENDING、callbacks/runtime/service=true/false/true`；只有完整locked tuple可见后，才把覆盖完整movement rect、位于交互UI下/VJ上的shield effective filter设为STOP并执行action/carrier clear。claimed VJ保持非IGNORE以接收旧touch-focus release/cancel，shield按冻结FSM以预分配`{shield_epoch,touch_index}` bank持有自己命中的touch直到matching release/cancel；`ACTION_CLEAR_FAILED`保持LOCK_PENDING tuple进入fault。IDLE loading cleanup始终保持`true/false/false`与PREACTIVE_DISCARD，不开启bank service；既有locked state保持`true/false/true`。capacity由全部支持artifact manifest的`max_concurrent_touches`上界冻结，任意稀疏index按entry存储。Host/shield callback不能写两个enable字段、safe-close、直接发布Input/GameRoot状态、写movement carrier或写四个movement actions；GameRoot的ALWAYS paused control pump在每次Paused/Resume main-loop iteration及choice/resume入口先调用`service_pending_input_fault`，在引擎恢复派发main-loop后的最迟下一iteration消费latch。
- participant 在 battle load 时按固定表注册，Active 后不可增删或重排。每项至少提供 `participant_id`、允许 phase 和无分配 `run_phase(phase,context,lease_id) -> int status`；相同输入下调用顺序稳定。
- GameRoot 不把 SpatialGrid lease 当安全 capability；它以 scene ownership 和私有引用保证只有自身调用 `begin_phase/end_phase`，consumer 只在当前同步调用栈内借用 primitive `lease_id`，不得缓存到下一 callback。

> **Input bootstrap gate**：BOOT/HOME中、任何battle input节点入树前，唯一GameRoot bootstrap恰调用一次`Input.set_use_accumulated_input(false)`并实时readback false，同时只读验证`ProjectSettings.get_setting("input_devices/buffering/agile_event_flushing", false)==false`；后者由`project.godot`冻结。随后在BOOT的Home/Battle input target、consumer、callback、Input polling与`window_input` consumer均未激活时恰调用一次`Input.flush_buffered_events()`；flush中的项目Control callback、scene transition、choice/movement/gameplay effect必须为0，四个movement action随后均`pressed=false/raw=0`，但不宣称Godot InputMap/cache/pressed状态零修改。本局后续Input setter、flush与ProjectSettings runtime mutation均为0。BATTLE_LOADING的initialize及每个ACTIVE `PRE_ACQUIRE`都再次验证Input accumulated实时readback与ProjectSettings agile冻结值，漂移返回`INVALID_CONFIG`，不得取得Viewport owner或写gate。Host先保持`UNARMED`，initialize成功后精确进入IDLE=`callbacks_armed=true、runtime_ingress_armed=false、shield_bank_service_enabled=false`；即使节点已在树，VJ不可命中，shield `_gui_input`只`accept_event()`并走PREACTIVE_DISCARD，项目owned gesture/bank/action/carrier/pending-status零写，跨ACTIVE仍held的旧手势不得晋升。这里不禁止引擎Viewport写handled/touch-focus。Host `_notification`在preactive期间只允许写预分配、无需trusted tick的`preactive_invalidation_latched + reason_bits`；APP_BACKGROUND/INPUT_GEOMETRY_CHANGED不得静默丢失。GameRoot只允许Input initialize在主线程无await/无用户callback区间最终重读geometry、完成验证并确认buffering/preactive条件后seed trusted tick、连接并保存exact epoch-bound Callable、原子arm callbacks；失败保持UNARMED且三字段均false、bank空、连接数0。initialize后若preactive invalidation到达，最终`IDLE→ACTIVE`返回`WRONG_STATE`且零开放，GameRoot按BATTLE_LOADING lifecycle abort逆序cleanup回PREP，不进入CONTROLLED_FAULT。

> **Viewport physical input gate**：唯一GameRoot也是目标battle Viewport `gui_disable_input`的唯一项目writer，预分配owner sentinel并执行固定阶段矩阵。loading固定runtime=false/service=false；resume从RESUME_LOCKED发布起至ACTIVE guard提交前固定runtime=false/service=true。`PRE_ACQUIRE`验证Input accumulated实时readback与ProjectSettings agile冻结值均为false、精确Viewport identity、gate=false与owner空闲；failure不写Viewport。验证成功才取得owner进入`SET_TRUE_IN_FLIGHT`并调用true setter；Godot在bool仍false时同步drop focus/leave/tooltip及可能的focused-Control `_gui_input`，此区间允许Viewport dispatch。项目生产callback只能latch invalidation/diagnostic且禁止直接注入InputEvent；若hostile harness或引擎入口仍在同步窗口交付合法ScreenTouch，loading由shield `accept_event()`后PREACTIVE_DISCARD且bank为0，resume由STOP shield按现有FSM `accept_event()`并更新held bank。两路VJ claim/action/generation、carrier、choice、gameplay consumer、顶层state及两个enable字段写次数均为0；resume bank内容变化发生在已经为true的shield service内。true返回/readback=true后才进入`GATE_HELD`，此后`push_input`在Control picking前返回；first observer、resume unpause/second observer、Input guard以及Input/GameRoot局部ACTIVE提交都位于该区间。resume first observer发现held时直接以`HELD_ONLY_RETURN_TO_DRAIN`恢复terminal路由，不得继续unpause/ACTIVE或升级technical fault；若press与matching terminal均在setter返回前使bank回空，则其他条件clean时可继续。两次observer均clean后，Input ACTIVE guard以单一局部提交原子翻转为runtime=true/service=false；唯一私有`release_viewport_input_gate(reason)`持有false setter直接call-site：`ACTIVATION_SUCCESS`在局部提交后开放；`HELD_ONLY_RETURN_TO_DRAIN`在first observer或unpause后的repause分支开放terminal路由；`LOAD_ABORT_OR_FAULT_CLEANUP`仅在battle input teardown且PREP/ControlledFault/Home UI成为唯一target后开放。每个owner周期false/clear各一次；pre-acquire failure release=0。静态审计禁止其他writer和生产activation callback中的`Input.parse_input_event`、`Input.flush_buffered_events`、`Viewport.push_input/push_unhandled_input`或等价注入；hostile harness仅用于证明containment。
...
| `CONTROLLED_FAULT` | 技术故障阻断层 | 停止 gameplay，不提交本局奖励；只允许安全返回洞府 |
...
BATTLE_LOADING固定顺序为：`BOOT freeze Input accumulated=false/read ProjectSettings agile=false→target-free flush historical buffer→Config build(battle_ready) → RNG System init(run_seed from snapshot，注册为 service participant，各流按固定表注册 stream_id 且 Active 后不可增删) → 以同一snapshot ID预分配authority/query/remap/movement carriers及immutable VJ candidate-config snapshot → Host/VJ/shield入树但UNARMED且input不可达 → InputSystem readback buffering、重读geometry并validate/preactive-latch check → seed trusted tick → exact epoch-bound Callable connect → atomically arm callbacks(state=IDLE,runtime_ingress=false,shield_bank_service=false) → SpatialGrid.init → BattlePoolSet.init → lifecycle owner warmup → 全体snapshot ID一致性预检 → PRE_ACQUIRE(buffering/preactive/Viewport identity/owner/gate check) → SET_TRUE_IN_FLIGHT(owner acquire + true setter callbacks) → true return/readback/GATE_HELD → 再次检查preactive invalidation → guarded InputSystem.publish_runtime_state(ACTIVE,tick_revision)原子提交Input state/runtime=true/service=false → GameRoot局部提交consumer/BATTLE_ACTIVE → release(ACTIVATION_SUCCESS) false返回并清owner`。UNARMED/armed-IDLE期间touch统一PREACTIVE_DISCARD，项目owned bank/gesture/action/carrier/pending-status写次数=0；resize/background只写preactive invalidation latch。`SET_TRUE_IN_FLIGHT`允许Viewport delivery但VJ/gameplay effect必须为0；只有`GATE_HELD`到false调用前要求Viewport/Control/VJ delivery=0。false返回后至首MOVEMENT_COMMIT之间的第一下新OS press必须可接受。buffering/identity/owner/gate的pre-acquire failure不写Viewport；preactive/activation invalidation命中则逆序cleanup，已持gate者在battle input节点移除且PREP成为唯一target后以`LOAD_ABORT_OR_FAULT_CLEANUP` release，未持gate者release=0，均不显示technical fault。其他任一步失败不开放input并按原failure路径cleanup。Input连接后失败须精确disconnect已连Callable并令callbacks/runtime/service均false，再clear movement carrier；若Grid已Active则先Grid teardown使handle域失效，再Pool teardown，最后Grid reset并释放snapshot。Pool init原子失败且尚无borrow时不得伪造部分READY。

> **第十七轮BATTLE_LOADING containment补充**：上述`SET_TRUE_IN_FLIGHT`的“VJ/gameplay effect=0”不包括shield自身的`accept_event()`引擎消费。hostile合法ScreenTouch必须由armed-IDLE shield走PREACTIVE_DISCARD，项目bank保持0且不得跨ACTIVE晋升；生产callback直接注入仍由静态guard拒绝。BOOT flush的项目零effect不等于Godot InputMap/cache零修改。

> **第十八轮BATTLE_LOADING service补充**：从initialize成功到guarded ACTIVE commit前，三字段始终精确为`callbacks/runtime/service=true/false/false`；setter-in-flight callback不得临时开启shield bank service，PREACTIVE_DISCARD不得写bank/FSM/first-error latch。guard clean只以单一局部提交变为`true/true/false`；任何loading abort最终teardown为`false/false/false`。
...
- 每次准备写 inactive authority bank（普通tick或Paused choice）前，**强制从当前 published bundle 全量复制到 inactive bank（O(count)，受 `MAX_INDEXED_ENTRIES` 上界），再 apply 本 tick mutations**；全量 copy 是契约（正确性优先，对齐项目 fail-fast/正确性优先 pillar），不是优化选项。**禁止 dirty-patch / 增量 patch 策略**——全量 copy 消除 stale-bank 残留（systems BL3）与 dirty-write 泄漏到 published bank 的隔离 hazard（TD R5：inactive 与 published 必须是不同对象引用，全量 copy 保证 inactive 的任何写都不改变 published 的可观察字段）。全量 copy 写入预分配 `PackedInt64Array`/SoA carrier，不创建 Array/Dictionary/RefCounted，故不违 AC-G1 零增长（成本为 CPU 时间非 allocation）。每次准备inactive resolution bank时先将count与`published_tick`清为未发布状态。
