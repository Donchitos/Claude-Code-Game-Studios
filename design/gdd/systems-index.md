...
|---|-------------|----------|----------|--------|------------|------------|
| 1 | GameRoot & Scene Flow | Core | MVP | In Review | design/gdd/game-root-scene-flow.md | 第三轮 full re-review 后 Re-review Pending：七 phase、safe-boundary pause、authority/resolution 双 token、Config run_seed/snapshot ID、RNG fault、Grid/Pool resume transaction、Input Viewport gate、RunOutcome/Save envelope。第三轮 blocker 已修订但尚未经第四轮独立复审；runtime/performance/evidence gates 仍 OPEN。详见 reviews/game-root-scene-flow-review-log.md |
| 2 | InputSystem | Core | MVP | In Review | design/gdd/input-system.md | GameRoot 第三轮传播修订：ALWAYS Host/shield/VJ 不再作为 `NOTIFICATION_UNPAUSED` 必达观察者，改为 `set_pause(false)` 返回后 GameRoot 显式 second observer；新增 InputSystem-owned Window/Viewport geometry relay。其余三字段、Viewport gate、held-drain 契约沿用；须独立复审。 |
...
| 6 | Stage & Map | Core | MVP | In Review | design/gdd/stage-map.md | Config snapshot（校验与域）；Stage scene拥有唯一Camera2D规格/identity，GameRoot BATTLE_LOADING只注册/注入。第三轮传播后须独立复审。 |
